...
const PreviousModal = ({ isOpen, setIsOpen, modalData, number, sReward, nftData, loading, stakeReward }) => {
  ...
  const unixEndTimestampString = `${Number(modalData?.stakingEndTime)}`;
  ...
  const { setMetaMaskLoader } = useStateContext1();

  // claim function 
  const handleClaim = async () => {
    setIsOpen(false);
    try {
      const response = await Claim(Number(modalData?.stakingId), wallet)
      if (response) {
        setMetaMaskLoader(false);
        Success({ message1: "Successfully Claimed", autoClose: true });
        return response;
      }
    } catch (error) {
      setMetaMaskLoader(false);
  ...
  const handleClaimAndTransfer = async () => {
    setIsOpen(false);
    try {
      setMetaMaskLoader(true);
      const response = await ClaimAndTransfer(Number(modalData?.stakingId), wallet)
      if (response.status === true) {
        setMetaMaskLoader(false);
        Success({
          message1: "Successfully Claimed & Transfered",
          autoClose: true,
        });
        stakeReward()
        return response;
      ...
      Error({ message: "Failed to stake transfer" });
      setMetaMaskLoader(false);
      throw error;
    } finally {
      setMetaMaskLoader(false);
  ...
  return (
    <div>
      {isOpen && (
        <div className="fixed  bg-black backdrop-blur-sm bg-opacity-[50%]  w-[full]  inset-0  flex items-center justify-center z-50">
          <div className=" bg-[#000F1F] border border-blue relative gap-3 p-8 flex flex-col justify-center items-center rounded-xl shadow-md w-[90%] lg:w-[44%]">
            <div className="flex items-center justify-center lg:mb-4">
              <h2 className="text-lg font-semibold lg:text-2xl text-yellow lg:font-bold">
                Previous Staking
              </h2>

              <button onClick={closeModal}>
                <svg
                  className="absolute w-6 text-gray-600 cursor-pointer fill-current top-4 right-5"
            ...
            <p className="text-xs text-center text-white md:text-sm lg:text-md">
              See previous rewards and details here. You can also claim your
              rewards and unstake you NFT’s here.
            </p>
            <hr className="h-[1px] w-[100%] text-white opacity-20" />

            <div className="flex flex-col items-center justify-center w-full gap-3">

              <div className="flex items-center justify-start w-full gap-6">
                <div className="flex ml-4  flex-col w-[33%] ">
                  <p className="text-xs text-left text-white md:text-sm lg:text-md">
                    Staking ID
                  </p>
                  <p className="mt-3 text-xs text-left text-white md:text-sm">
                    {" "}
                    {Number(modalData?.stakingId)}
                  </p>
                  <hr className="h-[1px] w-[90%]  text-white opacity-20" />
                </div>
                <div className="flex items-start  flex-col w-[33%] ">
                  <p className="text-xs text-white md:text-sm lg:text-md ">
                    Start Time
                  </p>
                  <p className="mt-3 text-xs text-white md:text-sm ">
                    {" "}
              ...
              <div className="flex items-center justify-start w-full gap-1">
                <div className="flex items-start ml-4 flex-col w-[33%]  ">
                  <p className="text-xs text-white md:text-sm lg:text-md ">
                    Claimable Reward
                  </p>
                  {loading || !wallet ? (
                ...
                <div className="flex items-start  flex-col w-[33%]  ">
                  ...
                  {loading || !wallet ? (
                    ...
                    <p className="text-xs text-white text-end md:text-sm lg:text-md">
                      {nftData || 0}
                    </p>
                  )}
                  <hr className="h-[1px] w-[80%] text-white opacity-20" />
            ...
            <div className="flex flex-col items-center justify-center w-full md:w-[50%] gap-3 pt-3 lg:flex-row">
              {/*<Button
                onClick={handleClaim}
                variant={"  text-yellow "}
                children={"Claim Reward"}
                tooltip={"Press button to claim your rewards"}
                disabled={nftData==0? true:false}
              />*/}
              <Button
                onClick={handleClaimAndUnstake}
                variant={" text-yellow "}
                children={"Claim Rewards and Unstake Bros"}
                tooltip={"Claim your rewards & Unstake NFT"}
                disabled={nftData == 0 ? true : false}
            ...
            {/*<Button
              onClick={handleClaimAndTransfer}
              variant={" text-black bg-yellow "}
              children={"Claim and Move to Next Staking "}
              tooltip={"Claim your rewards and move NFT to next Staking"}
              disabled={nftData==0? true:false}
